...
// Pre-generated list of 56 items corresponding to 001.jpg to 056.jpg
export const INITIAL_ARTWORKS: Artwork[] = Array.from({ length: 56 }, (_, index) => {
  ...
  const curatedUrl = `${GALLERY_IMAGE_BASE}/${filename}`;
  ...
  return {
    ...
    description: `Original ${category.toLowerCase()} crafted by Aanchal, recorded as catalog index #${num}.jpg. Emphasizes classic lighting, soft tonal transitions, and rich textural depth.`,
    dimensions: `${3200 + (index * 120)} x ${2400 + (index * 90)} px`,
    imageUrl: curatedUrl,
    highResUrl: curatedUrl,
    tags: ['AanchalArt', category, `Catalog-${num}`, index % 2 === 0 ? 'Vintage' : 'Studio', 'DigitalFineArt'],
